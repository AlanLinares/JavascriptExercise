...
//added numElev to variable list
    var numElev, numApp, numFloors, numBase, maxOcc;
    var prodRange = {
        type: null,
        price: null,
...
//changed the fixed pricing to call getProdRange() so it returns the proper pricing
//also rearranged the dollar sign
...
//standard was spelled wrong, fixed and now returns the values
...
//added getInfoNumApp() to use with residential calculations
    function GetInfos() {
        getInfoNumApp();
        getInfoNumFloors();
        getInfoNumBase();
...
//added $('numElev_2) to display N.ofElevator
    function emptyElevatorsNumberAndPricesFields() {
        $('#numElev_2').val('');
        $('#numElev_3').val('');
        $('.priceField').val('');
    };
//added numberElev which I believe started populating elevator numbers
...
//added an alert for negative floor inputs
    ...
    //deleted the $('numPark).val condition to get commercial call working
    //changed apiCall('commercial') to apiCall('corporate') for if($('corporate'))
    //also added an if for hybrid which uses apiCall('corporate') since the equations are the same
    function doCalc() {
